...
def main():
	...
	if args.dataset == "yelp":
		...
		filename = {'train':"ltrain_1129.json", "valid":"lvalid_1129.json", "test":"ltrain_1129.json"}
	else:
	...
	test_set = DataProcess(filename=filename['test'],fileprefix=dataprefix)
	test_iter = DataLoader(dataset=test_set, batch_size=1, shuffle=True)

	ignore = training_set.get_padding_index()
	beginning = trainning_set.get_beginning_index()
	...
	if args.test:
		...
		model.to(device)
		train.test(model, args, test_iter, path="./checkpoints_"+args.dataset+"/epoch_31.pt", beginninng_index=beginning)
	else:
